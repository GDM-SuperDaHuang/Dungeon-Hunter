# 网络
## Owner / Autonomous Proxy 机制
# Owner
1) 每个 Actor 在 UE 中都有一个 Owner
2) Owner 通常是 谁生成了这个 Actor / 谁负责它

    COND_OwnerOnly:只复制给Owner客户端
    bOnlyRelevantToOwner: Actor只对Owner客户端相关


    例子：
    玩家角色生成技能 Projectile → Owner = 玩家角色
    AI 生成火球 → Owner = AI

# Autonomous Proxy
1) UE 的网络模型中，每个 Actor 有一个 Role：
    ROLE_Authority（服务器权威）
    ROLE_AutonomousProxy（本地拥有权的客户端 Actor）
    ROLE_SimulatedProxy（普通客户端 Actor）

2) Autonomous Proxy：
    本地客户端认为自己“拥有”这个 Actor
    客户端可以预测 Actor 的行为（比如移动）
    服务器同步这个 Actor 时，客户端 通常不会重新生成 Actor，因为它已经有一个本地版本
